...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method m : allMethods) {
                if (m.getName().equals(route.getAction()) &&
                        Arrays.stream(m.getParameterTypes()).noneMatch(c -> c != Map.class && c != HttpServletRequest.class)) {
                    method = m;
                }
            }
        }
        ...
        try {
            ...
            for (int i = 0; i < types.length; i++) {
                if (types[i] == Map.class) {
                    ...
                    throw new ServletException("Unsupported method signature: '" + types[i] + "'.");
                }
            }
        ...
        Template template = newTemplateByLocale(pageSimpleName + ".ftlh", lang);
    ...
    private Template newTemplateByLocale(String templateName, String language) throws ServletException {
        return newTemplate(templateName, language == null ? Locale.ENGLISH : new Locale(language));
    }
