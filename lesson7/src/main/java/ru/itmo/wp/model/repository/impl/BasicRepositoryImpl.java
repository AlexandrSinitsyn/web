...
public abstract class BasicRepositoryImpl<T extends ModelObject> {
    ...
    public List<T> findByUserId(long userId) {
        List<T> res = new ArrayList<>();
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `" + tableName + "` WHERE userId=? ORDER BY id DESC")) {
    ...
    protected void save(T t, StatementSetter statementSetter, String... fields) {
        save(t, statementSetter, (e, gk) -> {}, fields);
    }
